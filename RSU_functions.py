...
def get_stock_price_euro(dt, criteo_stock_dict=criteo_stock_dict, exchange_rate_dict=exchange_rate_dict):
    stock_price_usd = get_value_on_date(dt, criteo_stock_dict)
    return stock_price_usd / get_value_on_date(dt, exchange_rate_dict)


def get_stock_price_euro_from_price_in_USD(dt, stock_price_USD, exchange_rate_dict=exchange_rate_dict):
    return stock_price_USD / get_value_on_date(dt, exchange_rate_dict)
...
def compute_tax_info_from_matched_transaction(
    selling_date_dt,
    vesting_date_dt,
    selling_price_USD,
    macron_law_id=0,
    ...
    tax_to_pay = (
        vesting_price_with_moins_value * ((1 - rebate) * tax_info_dict["TMI_IR"] + tax_info_dict["cotisation"])
        + plus_value * tax_info_dict["flat_tax_plus_value"]
    )
    return {
...
def get_sale_order_greedy(
    ...
    for i, event in enumerate(portfolio["available_stock"]):
        tax_info = compute_tax_info_from_matched_transaction(
            sell_event["date"],
            event["date"],
            sell_event["stock_unit_price_USD"],
            macron_law_id=event["macron_law_id"],
            tax_info_dict=tax_info_dict,
            criteo_stock_dict=criteo_stock_dict,
            exchange_rate_dict=exchange_rate_dict,
        )
        sale_tax_info.append({"position": i, "available_stock": event["amount"], "tax": tax_info["tax"]})
    ...
    for sale in ordered_sale:
        ...
        share_sold = min(
            portfolio["available_stock"][sale_position]["amount"],
...
def get_sale_order_from_optionality(
    sell_event,
    portfolio,
    tax_info_dict=tax_info_dict,
    criteo_stock_dict=criteo_stock_dict,
    ...
    for i, event in enumerate(portfolio["available_stock"]):
        ...
        score_for_sorting = (
            -np.abs(np.log(selling_price / tax_info["vesting_price"]))
            - 100 * (tax_info["rebate"] > 0)
            - 1000 * event.get("amount_already_declared", 0)
            + skip_sale_weight
        )
        sale_tax_info.append(
            {
                "position": i,
                "available_stock": event["amount"],
                "tax": tax_info["tax"],
    ...
    for sale in ordered_sale:
        ...
        if remaining_share_to_sell == 0:
            return sale_order
    if remaining_share_to_sell > 0:
        raise Exception("error, you did not have enough share to sell this amount, please check")


def get_sale_order_fifo(
    sell_event,
    portfolio,
    tax_info_dict=tax_info_dict,
    criteo_stock_dict=criteo_stock_dict,
...
def get_sales_result(
    ...
    for vested_action_sale in sale_order:
        vested_event = portfolio["available_stock"][vested_action_sale["position"]]
        tax_info = compute_tax_info_from_matched_transaction(
            sell_event["date"],
            vested_event["date"],
            sell_event["stock_unit_price_USD"],
        ...
        montant_global_516 = tax_info["selling_price"] * vested_action_sale["share_sold"]
        # frais separer egalement sur les differentes transactions
        frais_de_cession_517 = (
            sell_event["total_fee_dollars"] / get_value_on_date(sell_event["date"], exchange_rate_dict) / nb_sales
        )
        prix_de_cession_net_518 = montant_global_516 - frais_de_cession_517
        prix_daquisition_global = tax_info["vesting_price"] * vested_action_sale["share_sold"]
        ...
        sale_recap.append(
            {
                ...
                "resultat": resultat,
                "vesting_amount_with_moins_value": vested_action_sale["share_sold"]
                * tax_info["vesting_price_with_moins_value"],
                "rebate_with_moins_value": vested_action_sale["share_sold"]
                * tax_info["vesting_price_with_moins_value"]
                * tax_info["rebate"],
        ...
        portfolio["available_stock"][vested_action_sale["position"]]["amount"] = (
            portfolio["available_stock"][vested_action_sale["position"]]["amount"] - vested_action_sale["share_sold"]
        )
        if portfolio["available_stock"][vested_action_sale["position"]].get("amount_already_declared", 0) > 0:
            portfolio["available_stock"][vested_action_sale["position"]]["amount_already_declared"] = max(
                0,
